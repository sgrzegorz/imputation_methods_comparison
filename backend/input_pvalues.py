...
def achieve_min_pvalue(df,gene_name,rsy):
    min_val = 99999999

    for rs in rsy[gene_name]:
        try:
            candidate = df.loc[df['hm_rsid'] == rs, 'p_value'].iloc[0]
...
for i,record in enumerate(reader): #iteruj po wierszach vcf-a
    ...
    # iterujac po wierszach vcf-a dodawaj do przyporzadkowuj kazdej nazie genu jego rs-y
    snp = record.ID[0]
    genes.add(gene_name)
    if gene_name in rsy:
...
# print('Start finding minimal pvalue for each gene')
# min_pvalue = {}
# for gene in genes:
#     snps = rsy['gene']
#     if len(snps) == 0:
#         raise Exception(f'found 0 rs for gene, this situation should be handled earlier')
#     for i in snps:



print('Saving results')
with open("input_wyniki.txt", "w+") as f:
    for gene_name in genes:
        f.write(gene_name)
        f.write('\t')
        f.write(min_pvalues[gene_name])
        # for rs in rsy[gene_name]:
        #     f.write(rs)
        #     f.write('\t')
        f.write('\n')
