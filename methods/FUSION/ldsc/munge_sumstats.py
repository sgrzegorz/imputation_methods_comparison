...
def parse_dat(dat_gen, convert_colname, merge_alleles, log, args):
    ...
    for block_num, dat in enumerate(dat_gen):
        ...
        wrong_types = [c for c in dat.columns if
                       c in numeric_cols and not np.issubdtype(dat[c].dtype,
                                                               np.number)]
        ...
        if args.keep_maf:
            ...
            dat.drop(
                [x for x in ['INFO', 'FRQ'] if x in dat.columns], inplace=True,
                axis=1)
...
def allele_merge(dat, alleles, log):
    ...
    dat.loc[~jj.astype('bool'), [i for i in dat.columns if i != 'SNP']] = float(
        'nan')
    ...
    try:
        ...
        if args.signed_sumstats is not None or args.a1_inc:
            mod_default_cnames = {x: default_cnames[
                x] for x in default_cnames if
                                  default_cnames[x] not in null_values}
        else:
        ...
        for field in cname_translation:
            ...
            if numk > 1:
                raise ValueError('Found {num} columns named {C}'.format(C=field,
                                                                        num=str(
                                                                            numk)))
        ...
        for head in cname_translation.values():
            ...
            if numc > 1:
                raise ValueError(
                    'Found {num} different {C} columns'.format(C=head,
                                                               num=str(numc)))

        if (not args.N) and (not (args.N_cas and args.N_con)) and (
                'N' not in cname_translation.values()) and \
                (any(x not in cname_translation.values() for x in
                     ['N_CAS', 'N_CON'])):
            raise ValueError('Could not determine N.')
        if ('N' in cname_translation.values() or all(
                x in cname_translation.values() for x in ['N_CAS', 'N_CON'])) \
                and 'NSTUDY' in cname_translation.values():
            nstudy = [
                x for x in cname_translation if
                cname_translation[x] == 'NSTUDY']
            for x in nstudy:
        ...
        if args.merge_alleles:
            ...
            merge_alleles = pd.read_csv(args.merge_alleles,
                                        compression=compression, header=0,
            ...
            merge_alleles['MA'] = (
                    merge_alleles.A1 + merge_alleles.A2).apply(
                lambda y: y.upper())
            merge_alleles.drop(
        ...
        signed_sumstat_cols = [k for k, v in cname_translation.items() if
                               v == 'SIGNED_SUMSTAT']
        dat_gen = pd.read_csv(args.sumstats, delim_whitespace=True, header=0,
                              compression=compression,
                              usecols=cname_translation.keys(),
                              na_values=['.', 'NA'], iterator=True,
                              chunksize=args.chunksize,
                              dtype={c: np.float64 for c in
                                     signed_sumstat_cols})
        ...
        log.log(
            msg.format(M=len(dat), F=out_fname + '.gz',
                       N=dat.N.notnull().sum()))
        if p:
